<template>
 ...
 <div v-if="showWSCK" class="card">
      <div class="card-header">
        ...
        <div class="card-body text-base leading-6">
          <b>强烈推荐抓wskey</b>
          <p>wskey长期有效，不需要频繁更新，暂不清楚wskey失效时间，据说至少1年以上。</p>
          ...
          <p class="card-subtitle">wskey非录入后立马上线，系统会在指定时间检查wskey，有效则自动转换出cookie登录</p>
          <p class="card-subtitle">cookie失效后，也会在系统设定的指定时间内自动转换出新的cookie，实现一次录入长期有效</p>
          <p class="card-subtitle">详细抓包教程稍后更新，尽情期待</p>
        </div>
    ...
    <div v-if="showCK" class="card">
      <div class="card-header">
        ...
        <div class="card-body text-base leading-6">
          <p>PC用户建议使用开源工具<a href="https://github.com/Waikkii/JD_Get_Cookie" target="_blank" id="waikiki">JD_Get_Cookie</a>获取cookie并在下方填写。</p>
          <p>手机用户可以使用Alook浏览器登录<a href="https://m.jd.com/" target="_blank" id="jd">JD官网</a>，并在菜单-工具箱-开发者工具-Cookies中获取（Android和iPhone通用）。</p>
          <p>——安卓用户须手动提取pt_key和pt_pin，格式如："pt_key=xxxxxxxxxx;pt_pin=xxxxxx;"。</p>
          <p>——IOS用户可以使用快捷指令<a href="https://www.icloud.com/shortcuts/f0bf8b56bd0949afabfc00ca6fbfb4c2" target="_blank" id="getcookie">提取cookie</a>。</p>
          <p>显示“欢迎回来XXX”/“注册成功XXX”即更新/注册成功，不用理会红色提示框。</p>
        </div>
